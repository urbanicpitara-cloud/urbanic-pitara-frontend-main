      ...
      <div className="container mx-auto flex h-16 items-center justify-between px-4 md:px-6">
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          {user ? (
            ...
            <Link href="/auth">
              <span className="text-gray-900 font-medium hover:text-[var(--sage)] transition-colors text-sm uppercase tracking-wide">
                Login
              </span>
            </Link>
          )}
        ...
        <nav className="md:hidden bg-white border-t shadow-inner animate-slideDown">
          <div className="px-4 py-4 flex flex-col gap-4">
              ...
              <DropdownMenuContent className="min-w-[200px] shadow-md">
                <DropdownMenuItem asChild>
                  <Link
                    ...
                    href={`/search?q=${encodeURIComponent("t-shirt")}`}
                  >
                    T-Shirts
                  </Link>
                </DropdownMenuItem>
                <DropdownMenuItem asChild>
                  <Link
                    onClick={() => setMobileMenuOpen(false)}
                    href={`/search?q=${encodeURIComponent("men")}`}
                  >
                    All
                  </Link>
                </DropdownMenuItem>
            ...
            {user ? (
              ...
              <Link href="/auth" className="flex justify-center mt-4">
                <Button className="w-full bg-black text-white hover:bg-gray-800 font-medium rounded-none uppercase tracking-widest text-xs py-6">
                  Login / Register
                </Button>
              </Link>
            )}
