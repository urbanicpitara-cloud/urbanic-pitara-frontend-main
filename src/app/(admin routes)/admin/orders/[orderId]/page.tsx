          ...
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <ul className="space-y-4">
              {order.items?.map((item, idx) => {
                ...
                const imgUrl = item.product?.featuredImage?.url ?? "";
                const alt = item.product?.featuredImage?.altText ?? item.product?.title ?? "product";
                return (
                  <li key={item._id ?? item.id ?? idx} className="flex items-center gap-4 p-3 rounded-md hover:bg-gray-50 transition">
                    {imgUrl ? (
                      <div className="w-16 h-16 relative rounded-md overflow-hidden bg-gray-100">
                        <Image src={imgUrl} alt={alt} fill sizes="64px" className="object-cover" />
                      </div>
                    ) : (
                      <div className="w-16 h-16 rounded-md bg-gray-100 flex items-center justify-center text-sm text-gray-500">No image</div>
                    )}
